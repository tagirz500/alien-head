...
READ = """() => { const a = window.alien, t = a.track, q = b => b ? b.quaternion.toArray().map(v => +v.toFixed(3)) : null;
  return { face: t.haveFace, shoulders: t.haveShoulders, ms: Math.round(t.ms), yaw: +t.yawDeg.toFixed(1), pitch: +t.pitchDeg.toFixed(1), roll: +t.rollDeg.toFixed(1),
           ...
           hud: document.getElementById('hud').textContent, error: document.getElementById('error').textContent, status: document.getElementById('status').textContent }; }"""
async def run_clip(p, clip, tag):
    ...
    print(f"== {tag}: neutral yaw/pitch/roll {neutral['yaw']}/{neutral['pitch']}/{neutral['roll']} face {neutral['face']} shoulders {neutral['shoulders']} ms {neutral['ms']} eyes {neutral['eyes']} shoulder {neutral['shoulder']} gaze {neutral['gaze']}")
